...
$result = mysqli_query($con, "SELECT * FROM `$table_name` WHERE mac='$_SESSION[id]' AND DATE(last_updated) >= ( NOW() - INTERVAL 7 DAY )  order by last_updated DESC limit 1");
if ($result->num_rows >= 1) {
    ...
    $device = mysqli_query($con, "SELECT * FROM `$table_name` WHERE mac='$_SESSION[id]'");
    $_SESSION["user_type"] = ($device->num_rows >= 1) ? "repeat" : "new";
    mysqli_close($con);
}
